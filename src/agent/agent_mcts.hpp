...
namespace AlphaYa
{
	...
	MCTSAgent(SeedType seed,EvalType cc,IndexType s):rd(seed),c(cc),simulate_count(s){
		...
		std::cout<<"c "<<c<<std::endl;//patch
		std::cout<<"simulate_count "<<simulate_count<<std::endl;//patch
	}
		...
		class Node{
			...
			bool best_action(Action &action){
			...
			static std::shared_ptr<Node> explore(std::shared_ptr<Node>  node,const EvalType c,std::mt19937 &rd){
				...
				for(typename Node::Child &child:node->children){
					if(!child.node){
						...
						std::cout<<"aaaa children"<<std::endl;
				...
				for(const typename Node::Child &child:node->children){
					const std::shared_ptr<Node> &node=child.node;
					EvalType eval=((EvalType)(node->scores[player]))/((EvalType)(node->count))+k/std::sqrt((EvalType)(node->count));
					if(best<eval){
						best=eval;
						best_node=node;
					}
		...
		Action move(const State &state,std::istream &in, std::ostream &out) 
		{
			/*
			if(root){
			auto newroot=root->find_state(state);
			out<<"root = "<<root.get()<<std::endl;
			out<<"newroot = "<<newroot.get()<<std::endl;
			}*/
			if(root){
			root=find_state(root,state);
			}
			if(root){
				out<<"REUSE NODE"<<std::endl;//patch
			root->father.reset();
			}else{
				out<<"NOT REUSE NODE"<<std::endl;//patch
			...
			for(IndexType i=0;;++i){
				...
				out<<i<<" ROOT OK"<<std::endl;
				do{
					p=explore(p,c,rd);
				}while(!p->is_final);
				out<<"EXPLORE OK"<<std::endl;
				...
				//	if(i==0){debug(root);return root->children[0].action;}//patch
				//if(i==0)debug(root);//patch
				Action new_action;
